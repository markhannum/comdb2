...
static int refresh_reverse_conn_hosts() {
    ...
    rc = snprintf(cmd, sizeof(cmd), "exec procedure sys.physrep.get_reverse_hosts('%s', '%s', '%s', '%s')", gbl_dbname,
    ...
    if ((rc = cdb2_run_statement(repl_metadb, cmd)) == CDB2_OK) {
        while ((rc = cdb2_next_record(repl_metadb)) == CDB2_OK) {
            ...
            char **class_mach_list;
            const char **cluster_mach_list;
            int count = 0;
            ...
            if (is_valid_mach_class(host)) {
                if (class_machs(dbname, host, &count, &class_mach_list) == 0) {
                    ...
                    for (int i = 0; i < count; i++) {
                        if (add_reverse_host(dbname, class_mach_list[i], &new_reverse_conn_hosts) != 0) {
                            for (int j = 0; j < count; j++) {
                                free(class_mach_list[j]);
                            }
