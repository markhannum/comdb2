...
int bdb_check_files_on_disk(bdb_state_type *bdb_state, const char *tblname, int *bdberr)
{
    ...
    while ((error = bb_readdir(dirp, buf, &ent)) == 0 && ent != NULL) {
        ...
        logmsg(LOGMSG_USER, "%s checking tblname %s version %llu\n",
        ...
        if (rc == 1) {
            logmsg(LOGMSG_USER, "%s: bdb_is_new_sc_file returns true for %s version %llu\n", __func__, tblname, file_version);
            found_in_llmeta = 1;
            rc = 0;
        } else if (rc) {
            logmsg(LOGMSG_ERROR,
        ...
        if (!found_in_llmeta) {
            rc = bdb_process_each_table_dta_entry(bdb_state, NULL, tblname,
                                                  file_version, bdberr);
            if (rc == 1) {
                logmsg(LOGMSG_USER, "%s: bdb_process_each_dta returns true for %s version %llu\n", __func__, tblname, file_version);
                found_in_llmeta = 1;
                rc = 0;
            } else if (rc) {
                logmsg(LOGMSG_ERROR,
        ...
        if (!found_in_llmeta) {
            rc = bdb_process_each_table_idx_entry(bdb_state, NULL, tblname,
                                                  file_version, bdberr);
            if (rc == 1) {
                logmsg(LOGMSG_USER, "%s: bdb_process_each_idx returns true for %s version %llu\n", __func__, tblname, file_version);
                found_in_llmeta = 1;
                rc = 0;
            } else if (rc) {
                logmsg(LOGMSG_ERROR,
        ...
        logmsg(LOGMSG_USER, "%s: %s version %llu adding to oldfiles\n",
                    __func__, tblname, file_version);
