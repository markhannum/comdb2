...
static char msgtrap_fifo[128] = "/home/mponomar/tmp/msgtrap.sockpool";
...
int discover_passthrough(char *typestr);
...
void *client_thd(void *voidarg)
{
    ...
    while (1) {
        ...
        if (request == SOCKPOOL_DONATE) {
            ...
            if (newfd == -1 && 1) {
                newfd = discover_passthrough(typestr);
                if (newfd == -1) {
                    LOCK(&gbl_port_hints_lock)
                    {
                        if (!gbl_exiting) {
            ...
            UNLOCK(&gbl_port_hints_lock);
        } else if (request == SOCKPOOL_RESOLVE) {
            extern int discover(int fd, char *resquest);
            discover(fd, msg0.typestr);
        } else {
...
static void message_trap(int ntoks, char *toks[])
{
    if (ntoks == 0) {
        ...
        UNLOCK(&gbl_port_hints_lock);
    } else if (strcasecmp(toks[0], "discover") == 0) {
        discover_msg(ntoks, toks);
    } else {
...
static int cdb2_waitft()
{
    ...
    while (1) {
        ...
        extern void discover_fallback_check(void);
        discover_fallback_check();
        close(fd);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    extern void cached_destinations_init(void);
    cdb2_disable_sockpool();
    cached_destinations_init();
