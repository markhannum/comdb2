	...
	int online;
{
	...
	if (report_now != report_last && IS_REP_CLIENT(dbenv) && gbl_periodic_rep_report) {
		report_last = report_now;
		int inmem_sz = repdb_inmem_size();
		if (dbenv->coherency_check_callback && dbenv->coherency_check_callback(dbenv->coherency_check_usrptr) == 0) {
		  ...
		  rep->stat.st_gen, rep->stat.st_log_duplicated, inmem_sz /*rep->stat.st_log_queued*/, 
