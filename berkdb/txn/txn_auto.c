	...
	void *notused3;
{
	...
	if (argp->lflags & DB_TXN_PARTICIPANT)
		printf("DB_TXN_PARTICIPANT ");
	if (argp->lflags & DB_TXN_COORDINATOR)
		printf("DB_TXN_COORDINATOR ");
	printf("\n");
...
/*
 * PUBLIC: int __txn_regop_dist_log __P((DB_ENV *, DB_TXN *, DB_LSN *,
 * PUBLIC:     u_int64_t *, u_int32_t, u_int32_t, u_int32_t, u_int64_t, u_int64_t,
 * PUBLIC:     u_int64_t, u_int32_t, const DBT *, const DBT *, u_int32_t, u_int64_t,
 * PUBLIC:     const DBT *, void *usr_ptr));
 */
int
__txn_regop_dist_log(dbenv, txnid, ret_lsnp, ret_contextp, flags, opcode,
        generation, dtranid, context, timestamp, lflags, coordinator_name,
        coordinator_stage, coordinator_gen, sequence, locks,
        usr_ptr)
	DB_ENV *dbenv;
	DB_TXN *txnid;
	DB_LSN *ret_lsnp;
    ...
    void *usr_ptr;
{
...
#ifdef __txn_DEBUG
	fprintf(stderr,"__txn_regop_dist_log: begin\n");
#endif
	...
	logrec.size = sizeof(rectype) + sizeof(txn_num) + sizeof(DB_LSN)
	    ...
	    + sizeof(u_int64_t) /* dtranid */
	    + sizeof(u_int64_t) /* context */
	    + sizeof(u_int64_t) /* timestamp */
	    + sizeof(u_int32_t) /* lflags */
	    + sizeof(u_int32_t) + (coordinator_name == NULL ? 0 : coordinator_name->size) /* coordinator_name */
	    + sizeof(u_int32_t) + (coordinator_stage == NULL ? 0 : coordinator_stage->size) /* coordinator_stage */
	    + sizeof(u_int32_t) /* coordinator_gen */
        + sizeof(u_int64_t)
	    + sizeof(u_int32_t) + (locks == NULL ? 0 : locks->size); /* locks */
...
do_malloc:
    ...
    {
		LOGCOPY_32(bp, &coordinator_stage->size);
		bp += sizeof(coordinator_stage->size);
		memcpy(bp, coordinator_stage->data, coordinator_stage->size);
		bp += coordinator_stage->size;
    }
...
#ifdef LOG_DIAGNOSTIC
	if (ret != 0)
		(void)__txn_regop_dist_print(dbenv,
		    (DBT *)&logrec, ret_lsnp, (db_recops)0 , NULL);
#endif
...
#ifdef HAVE_REPLICATION
/*
 * PUBLIC: int __txn_regop_dist_getpgnos __P((DB_ENV *, DBT *,
 * PUBLIC:     DB_LSN *, db_recops, void *));
 */
int
__txn_regop_dist_getpgnos(dbenv, rec, lsnp, notused1, summary)
	DB_ENV *dbenv;
	DBT *rec;
	DB_LSN *lsnp;
	db_recops notused1;
...
#ifdef HAVE_REPLICATION
/*
 * PUBLIC: int __txn_regop_dist_getallpgnos __P((DB_ENV *, DBT *,
 * PUBLIC:     DB_LSN *, db_recops, void *));
 */
int
__txn_regop_dist_getallpgnos(dbenv, rec, lsnp, notused1, summary)
	DB_ENV *dbenv;
	DBT *rec;
	DB_LSN *lsnp;
	db_recops notused1;
...
/*
 * PUBLIC: int __txn_regop_dist_read_int __P((DB_ENV *, void *,
 * PUBLIC:     int do_pgswp,  __txn_regop_dist_args **));
 */
int
__txn_regop_dist_read_int(dbenv, recbuf, do_pgswp, argpp)
	DB_ENV *dbenv;
	void *recbuf;
	int do_pgswp;
	__txn_regop_dist_args **argpp;
{
	__txn_regop_dist_args *argp;
	u_int32_t uinttmp;
	u_int64_t uint64tmp;
...
#ifdef __txn_DEBUG
	fprintf(stderr,"__txn_regop_dist_read_int: begin\n");
#endif

	if ((ret = __os_malloc(dbenv,
	    sizeof(__txn_regop_dist_args) + sizeof(DB_TXN), &argp)) != 0)
		return (ret);
	argp->txnid = (DB_TXN *)&argp[1];
	...
	memset(&argp->coordinator_stage, 0, sizeof(argp->coordinator_stage));
	LOGCOPY_32(&argp->coordinator_stage.size, bp);
	bp += sizeof(u_int32_t);
	argp->coordinator_stage.data = bp;
	bp += argp->coordinator_stage.size;

    LOGCOPY_32(&uinttmp, bp);
    argp->coordinator_gen = (u_int32_t)uinttmp;
    bp += sizeof(uinttmp);

    LOGCOPY_64(&uint64tmp, bp);
    argp->sequence = (u_int64_t)uint64tmp;
    bp += sizeof(uint64tmp);
...
/*
 * PUBLIC: int __txn_regop_dist_print __P((DB_ENV *, DBT *, DB_LSN *,
 * PUBLIC:     db_recops, void *));
 */
int
__txn_regop_dist_print(dbenv, dbtp, lsnp, notused2, notused3)
	DB_ENV *dbenv;
	DBT *dbtp;
	DB_LSN *lsnp;
	db_recops notused2;
	void *notused3;
{
	...
	if ((ret = __txn_regop_dist_read_int(dbenv, dbtp->data, 0, &argp)) != 0)
		return (ret);
	(void)printf(
	    "[%lu][%lu]__txn_regop_dist%s: rec: %lu txnid %lx prevlsn [%lu][%lu]\n",
	    (u_long)lsnp->file,
	    (u_long)lsnp->offset,
	    (argp->type & DB_debug_FLAG) ? "_debug" : "",
	    (u_long)argp->type,
	...
	(void)printf("\tgeneration: %u\n", argp->generation);
    unsigned long long flipcontext;
    int *fliporig = (int *)&argp->dtranid;
    int *flipptr = (int *)&flipcontext;
    flipptr[0] = htonl(fliporig[1]);
    flipptr[1] = htonl(fliporig[0]);
    fflush(stdout);
    (void)printf("\tdtranid: %016lx %016llx\n", argp->dtranid, flipcontext);
    fflush(stdout);
    fliporig = (int *)&argp->context;
    flipptr = (int *)&flipcontext;
    flipptr[0] = htonl(fliporig[1]);
    flipptr[1] = htonl(fliporig[0]);
    fflush(stdout);
    ...
    (void)printf("\tcoordinator_name: %s", (char *)argp->coordinator_name.data);
    (void)printf("\tcoordinator_stage: %s", (char *)argp->coordinator_stage.data);
    (void)printf("\tcoordinator_gen: %u", argp->coordinator_gen);

	(void)printf("\tsequence: %"PRIu64"\n", argp->sequence);
...
/*
 * PUBLIC: int __txn_regop_dist_read __P((DB_ENV *, void *,
 * PUBLIC:      __txn_regop_dist_args **));
 */
int
__txn_regop_dist_read(dbenv, recbuf, argpp)
	DB_ENV *dbenv;
	void *recbuf;
	__txn_regop_dist_args **argpp;
{
	return __txn_regop_dist_read_int (dbenv, recbuf, 1, argpp);
}
 ...
 */
int
__txn_prepare_log(dbenv, txnid, ret_lsnp, flags, opcode, generation,
        dtranid, timestamp, coordinator_name, coordinator_stage,
        coordinator_gen, locks)
	DB_ENV *dbenv;
	DB_TXN *txnid;
	DB_LSN *ret_lsnp;
	u_int32_t flags;
	...
	void *notused3;
{
	...
	if ((ret = __txn_prepare_read_int(dbenv, dbtp->data, 0, &argp)) != 0)
		return (ret);
	(void)printf(
	    "[%lu][%lu]__txn_prepare%s: rec: %lu txnid %lx prevlsn [%lu][%lu]\n",
	    (u_long)lsnp->file,
	    (u_long)lsnp->offset,
	    (argp->type & DB_debug_FLAG) ? "_debug" : "",
	    (u_long)argp->type,
    ...
    (void)printf("\tcoordinator_name: %s", (char *)argp->coordinator_name.data);
    (void)printf("\tcoordinator_stage: %s", (char *)argp->coordinator_stage.data);
	(void)printf("\tcoordinator_gen: %u\n", argp->coordinator_gen);
	(void)printf("\tlocks: \n");
